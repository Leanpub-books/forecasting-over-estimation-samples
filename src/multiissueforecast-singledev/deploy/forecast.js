...
class HistoricalRecord {
    StartedOn;
    FinishedOn;
    constructor(StartedOn, FinishedOn){
...
function LoadHistory(sourceFilename, delimiter = ";") {
    ...
    function CreateHistory(csv) {
        ...
        for(let i = 1; i < csv.length; i++){
            let rec = new HistoricalRecord(parse1(csv[i].Cols[0], "yyyy-MM-dd"), parse1(csv[i].Cols[1], "yyyy-MM-dd"));
            records.push(rec);
        }
        return records;
    }
...
function Simulate(historicalData, numberOfSimulations, numberOfRandomSamples, aggregate) {
    const simulations = [];
